...
my $bleu_1 =  exp((my_log( $bleu[1] )));

my $bleu_2 =  exp((my_log( $bleu[1] ) +
                   my_log( $bleu[2] ) ) / 2) ;

my $bleu_3 =  exp((my_log( $bleu[1] ) +
                   my_log( $bleu[2] ) +
                   my_log( $bleu[3] ) ) / 3) ;

my $bleu_4 =  exp((my_log( $bleu[1] ) +
                   my_log( $bleu[2] ) +
                   my_log( $bleu[3] ) +
                   my_log( $bleu[4] ) ) / 4) ;

printf "BLEU = %.1f/%.1f/%.1f/%.1f (mBLEU=%.1f, BP=%.3f, ratio=%.3f, hyp_len=%d, ref_len=%d)\n",
    100*$bleu_1,
    100*$bleu_2,
    100*$bleu_3,
    100*$bleu_4,
